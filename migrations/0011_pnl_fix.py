    ...
    def forwards(self, orm):
        if not db.dry_run:
            for obj in orm.tradetransaction.objects.all():
                if obj.success and obj.close_price:
                    if(('final' in obj.data) and ('payout' in obj.data['final'])):
                        obj.payout = Decimal(obj.data['final']['payout'])
                    obj.pnl = obj.payout - obj.amount
                else:
                    obj.payout = obj.pnl = 0
                obj.save()
